...
use tokio::sync::mpsc;
use wednesday_model::{error::DataError, events::{MarketEvent}, identifiers::{Identifier, SubscriptionId}, instruments::Instrument, orderbook::OrderBook};
    ...
    Updater::Update: Identifier<Option<SubscriptionId>> + for<'de> Deserialize<'de>,
{
    async fn new(
        ...
        let (subscription_ids, init_book_requests): (Vec<_>, Vec<_>) = map
            .0
            .into_iter()
            .map(|(subscription_id, instrument)| {
                (
                    subscription_id,
