...
impl Identifier<Option<SubscriptionId>> for BinanceFuturesTrade {
    fn id(&self) -> Option<SubscriptionId> {
        Some(self.subscription_id.clone())
    }
}

impl From<(ExchangeId, Instrument, BinanceFuturesTrade)> for MarketIter<PublicTrade> {
    fn from((exchange_id, instrument, trade): (ExchangeId, Instrument, BinanceFuturesTrade)) -> Self {
        Self(vec![Ok(MarketEvent {
            exchange_ts: trade.trade_ts,
            local_ts: Utc::now(),
...
/// Deserialize a [`BinanceFuturesTrade`] "s" (eg/ "BTCUSDT") as the associated [`SubscriptionId`]
/// (eg/ "@trade|BTCUSDT").
...
/// Deserialize a [`BinanceFuturesTrade`] "buyer_is_maker" boolean field to a Barter [`Side`].
///
/// Variants:
...
mod tests {
    ...
    mod de {
        ...
        fn test_binance_trade() {
            struct TestCase {
                input: &'static str,
                expected: Result<BinanceFuturesTrade, SocketError>,
            }
            ...
            let tests = vec![
                TestCase {
                    ...
                    "#,
                    expected: Ok(BinanceFuturesTrade {
                        subscription_id: SubscriptionId::from("@trade|ETHUSDT"),
                        trade_ts: datetime_utc_from_epoch_duration(Duration::from_millis(1749354825200)),
                        id: 1000000000,
                        price: 10000.19,
                        amount: 0.239000,
            ...
            for (index, test) in tests.into_iter().enumerate() {
                let actual = serde_json::from_str::<BinanceFuturesTrade>(test.input);
